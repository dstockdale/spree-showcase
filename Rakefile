...
task :test_app do
  ...
  class SpreeShowcaseTestAppGenerator < Spree::Generators::TestAppGenerator

    def install_gems
      inside "test_app" do
        run 'bundle exec rake spree_core:install'
        run 'bundle exec rake spree_showcase:install'
      end
    end
...
gem 'spree_showcase', :path => \'#{File.dirname(__FILE__)}\'
      gems
    end

  end
  SpreeShowcaseTestAppGenerator.start
end
